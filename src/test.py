...
def get_batch_metrics(outputs, targets, bin_edges, batch_size:int, args)->list:
    ...
    for idx in range(batch_size):        
        ...
        # target_left, target_right, target_theta, target_rho = bin_normal2leftright(target_theta, target_rho, bin_edges) 
...
def main(args) -> None:
    ...
    for idx, checkpoint_path in enumerate(checkpoint_paths):
        detection_errors, theta_AEs, rho_AEs, theta_metrics, rho_metrics,\
            joint_metrics, nll_theta, nll_rho = [], [], [], [], [], [],  [], []
        theta_SE, rho_SE = 0, 0
        theta_cdfs, rho_cdfs = torch.zeros(1, device=config.device), torch.zeros(1, device=config.device)
        
        path = checkpoint_path.split('/')[9]  # fix this according to the path!!!!
        ...
        with torch.no_grad():
            while batch_data is not None:
                ...
                theta_top1, theta_top5 = accuracy(output.theta, theta_id, topk=(1, 5))  
                rho_top1, rho_top5 = accuracy(output.rho, rho_id, topk=(1, 5))  
                ...
                # NLL 
                nll_theta.append(get_nll(output.theta, theta_id, batch_size).cpu().numpy())
                nll_rho.append(get_nll(output.rho, rho_id, batch_size).cpu().numpy())
                
                batch_theta_cdf = get_cdf(get_prob(output.theta, args.activation), theta_id)
                batch_rho_cdf = get_cdf(get_prob(output.rho, args.activation), rho_id)
        ...
        _LOG.info('RMSE theta {} rho {} \n NLL theta {} rho {}'.format(theta_RMSE, rho_RMSE, nll_theta, nll_rho))
        ...
        metrics.update((theta_RMSE, nll_theta, theta_ause, theta_ece), 'theta')
        metrics.update((rho_RMSE, nll_rho, rho_ause, rho_ece), 'rho')
